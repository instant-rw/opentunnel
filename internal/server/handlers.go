...
func (s *Server) Logout(w http.ResponseWriter, r *http.Request) {
	...
	clearAuthCookies(w, s.config.SecureCookies)
	w.WriteHeader(http.StatusNoContent)
}
...
func (s *Server) CreateDeviceAuthorization(w http.ResponseWriter, r *http.Request) {
	...
	verificationURI := strings.TrimRight(s.config.BaseURL, "/") + "/device"
...
func (s *Server) establishSession(w http.ResponseWriter, r *http.Request, userID uuid.UUID) bool {
	...
	http.SetCookie(w, &http.Cookie{
		...
		Secure: s.config.SecureCookies, SameSite: http.SameSiteLaxMode, Expires: expiresAt,
	})
	http.SetCookie(w, &http.Cookie{
...
func clearAuthCookies(w http.ResponseWriter, secure bool) {
	for _, name := range []string{sessionCookie, csrfCookie} {
		http.SetCookie(w, &http.Cookie{
			Name: name, Path: "/", HttpOnly: name == sessionCookie, Secure: secure,
